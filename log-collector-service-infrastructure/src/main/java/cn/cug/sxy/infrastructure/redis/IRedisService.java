...
public interface IRedisService {
     ...
     * @param expired 过期时间
     */
    <T> void setValue(String key, T value, long expired, TimeUnit timeUnit);

    <T> void setValueIfAbsent(String key, T value, long expired, TimeUnit timeUtil);
    ...
    <T> void setList(String key, RList<T> values);
    ...
    /**
     * 执行管道操作
     * @param consumer 管道操作消费者
     */
    void executePipelined(Consumer<RBatch> consumer);
     ...
     * @return 执行结果
     */
    <T> T executePipelined(Function<RBatch, T> consumer);

    RAtomicLong getAtomicLong(String key);

    RKeys getKeys();

    <T> List<T> mget(List<String> keys);
